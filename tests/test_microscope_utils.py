...
class TestGroupPartialVolumeFiles:
    ...
    def test_with_partial_volumes(self):
        ...
        # Find which group has parts
        group_with_parts = None
        group_without_parts = None
        for group in groups:
            if len(group) == 3:
                group_with_parts = group
            elif len(group) == 1:
                group_without_parts = group

        assert group_with_parts is not None, "Should have one group with 3 files"
        assert group_without_parts is not None, "Should have one group with 1 file"
        assert 'volume1.tif' in group_with_parts
        assert any('_part' in f for f in group_with_parts)
    ...
    def test_multiple_volumes_with_parts(self):
        ...
        # Check that we have one group with 2 files and one with 3
