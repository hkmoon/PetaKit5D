...
class TestBwnHood3D:
    ...
    def test_use_in_morphology(self):
        ...
        # For morphological operations, we need to include the center
        hood6 = bwn_hood_3d(6)
        hood6_with_center = hood6.copy()
        hood6_with_center[1, 1, 1] = True  # Add center for dilation

        dilated = ndimage.binary_dilation(img, structure=hood6_with_center)

        # Should have the center and the 6 face neighbors (7 total)
        assert np.sum(dilated) == 7

        # Check center is True
        assert dilated[2, 2, 2] == True
        # Check at least some neighbors are True
        assert dilated[2, 2, 1] == True or dilated[2, 2, 3] == True
