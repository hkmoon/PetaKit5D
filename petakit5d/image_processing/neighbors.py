...
def bwn_hood_3d(conn: Literal[6, 18, 26] = 26) -> np.ndarray:
    ...
    if conn == 6:
        ...
        # All neighbors except corners (not center, not 8 corners)
        # Total = 26 - 1 (center) - 8 (corners) = 17... Actually 26 - 8 (corners) = 18
        # 3x3x3 has 27 elements total, indices 0-26
        # Corners are at: 0, 2, 6, 8, 18, 20, 24, 26
        all_indices = set(range(27))
        all_indices.discard(13)  # Remove center
        corners = {0, 2, 6, 8, 18, 20, 24, 26}
        indices = list(all_indices - corners)
        nhood.flat[indices] = True
